#!/usr/bin/env python3
"""Symlink P-spectrogram PNGs into the same train/val/test ids as magnitude images."""
...
def main() -> None:
    ...
    src_root = processed / "pspec"
    missing = 0
    n = 0
    for split in ("train", "val", "test"):
        mag_dir = processed / "images" / split
        out_dir = processed / "pspec" / split
        out_dir.mkdir(parents=True, exist_ok=True)
        for mag in sorted(mag_dir.glob("*.png")):
            src = src_root / f"{mag.stem}.png"
            if not src.exists():
                missing += 1
                continue
            ensure_link(src, out_dir / mag.name)
            n += 1
    print(f"linked {n} pspec images, missing {missing}")
